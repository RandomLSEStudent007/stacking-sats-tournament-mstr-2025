...
def check_cells_identical(
    template_path, test_path, python_cell_numbers_to_check, expected_python_cell_count=3
):
    """
    ...
    if results["python_count_valid"]:
        for python_cell_num in python_cell_numbers_to_check:
            ...
            template_source = extract_python_cell_source_by_python_index(
                template_nb, python_cell_num
            )
            test_source = extract_python_cell_source_by_python_index(
                test_nb, python_cell_num
            )
            ...
            if template_hash != test_hash:
                ...
                if template_source is None:
                    ...
                    print("     Both cells exist but content differs")
            else:
                print(f"  ✅ Python Cell {python_cell_num} is identical")
            print()
    else:
        print(
            "⚠️  Skipping individual cell comparison due to incorrect Python cell count"
        )
        print()
...
def main():
    ...
    results = check_cells_identical(
        template_path, test_path, python_cells_to_check, expected_python_cell_count
    )
    ...
    if results["all_identical"]:
        ...
        if results["differences_found"]:
            for python_cell_num in results["differences_found"]:
                print(
                    f"   - Python Cell {python_cell_num} content differs between notebooks"
                )
